...
def prepare(path, mode=None, duration=None):
            ...
            for event_index, event in enumerate(cfg['timeline'], 1):
                ...
                for action in event['actions']:
                    ...
                    if op == 'clear':
                        ...
                        animations.append(Transform(objects[ident], build(action['object'])))
                    elif op == 'remove':
...
def main():
    ...
    if args.inside:
        ...
        return
    cfg = prepare(args.config, args.mode, args.duration)
    if args.validate:
